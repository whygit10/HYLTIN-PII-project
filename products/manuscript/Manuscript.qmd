...
```{r Load Raw Data}
d1 <- read.csv(here('data','raw-data','Crime_Reports_20240604.csv'))
```
...
head(d1)
```
The data several data and time fields, many of which are character strings meaning they will need to be cleaned/recoded. The rest of the fields are mostly classification variables and location information, but with 2,470,867 there is quite a bit of opportunity for frequency and classification analysis.
